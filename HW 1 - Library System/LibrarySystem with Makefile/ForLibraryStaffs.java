...
public class ForLibraryStaffs extends LibraryOperations{
        ...
        try{
            ...
            while (Read2.hasNextLine())
            {
                ...
                for (int i =0;i<Staff.RegisteredUserNum;++i)
                    if (Staff.UserArr[i].GetUserID() == Integer.parseInt(SplitArr2[0]))
                        IndexofUserID = i;
                ...
                Staff.UserArr[IndexofUserID].AddBorrowedBook(BookArr.get(IndexofBookID));
            }
        }
        catch (Exception e){
    ...
    public void RemoveBook(LibrarySystemsUsers Officer,String BookName) throws Exception
    {
        ...
        for (int i =0;i<Staff.RegisteredUserNum;++i)
            for(int j = 0;j<Staff.UserArr[i].GetBorrowedBooks().size();++j)
                if (Staff.UserArr[i].GetBorrowedBooks().get(j).GetName().equals(BookName))
                {
