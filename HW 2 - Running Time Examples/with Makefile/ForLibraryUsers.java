        ...
        try (BufferedWriter bWriter = new BufferedWriter(fileWriter)) {
            bWriter.write("USER ID;USER NAME;BOOK ID;BOOK NAME;");
             for (int i = 0;i<Staff.RegisteredUserNum;++i)
                 for(int j =0;j<Staff.UserArr.get(i).GetBorrowedBooks().size();++i)
                    bWriter.write("\n"+Staff.UserArr.get(i).GetUserID()+";"+Staff.UserArr.get(i).GetName()+";"+Staff.UserArr.get(i).GetBorrowedBooks().get(j).GetBookID()+";"+Staff.UserArr.get(i).GetBorrowedBooks().get(j).GetName() +";");
                bWriter.close();
        }
    }
}
