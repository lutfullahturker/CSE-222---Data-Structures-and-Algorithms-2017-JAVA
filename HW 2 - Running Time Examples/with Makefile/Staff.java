...
public class Staff implements LibrarySystemsUsers{
     ...
     */
    protected static LinkedList<User> UserArr = new LinkedList<>();
    ...
    private int FindMaxID() throws Exception
    {
        if (RegisteredUserNum == 0)
            return 10000;
        int max = UserArr.get(0).GetUserID();
        for (int i =0;i<RegisteredUserNum;++i)
        {
            if (UserArr.get(i).GetUserType() == 0)
            {
                if (UserArr.get(i).GetUserID() > max)
                    max = UserArr.get(i).GetUserID();
            }
            else {
                StaffIDCount = UserArr.get(i).GetUserID();
                StaffID = UserArr.get(i).GetUserID();
            }
        }
        return max;
    }
            ...
            while (Read.hasNextLine())
            {
                ...
                if (Integer.parseInt(SplitArr[2]) == 0)
                {
                    /*if (RegisteredUserNum >= CapacityOfUserArr)
                    {
                        ...
                        for (int i=0;i<RegisteredUserNum;++i)
                            UserArr.set(i, UserArr.get(i));
                        CapacityOfUserArr *= 2 ;
                        UserArr = new User[CapacityOfUserArr];
                        for (int i=0;i<RegisteredUserNum;++i)
                            UserArr.set(i, temp[i]);
                    }*/
                    UserArr.add(new User(SplitArr[1]));
                    UserArr.get(RegisteredUserNum).UserID = Integer.parseInt(SplitArr[0]);
                    ++RegisteredUserNum;  
                }
    ...
    public void AddUser(String UserName) throws Exception{
        ...
        UserArr.add(new User(UserName));
        SetUserID(UserArr.get(RegisteredUserNum));
        ++RegisteredUserNum;  
        ...
        try (BufferedWriter bWriter = new BufferedWriter(fileWriter)) {
            bWriter.write("\n"+UserArr.get(RegisteredUserNum-1).GetUserID()+";"+UserArr.get(RegisteredUserNum-1).GetName()+";"+UserArr.get(RegisteredUserNum-1).GetUserType()+";");
            bWriter.close();
        }
    }
